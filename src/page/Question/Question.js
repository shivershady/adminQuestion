            ...
            <Link className='text-sm text-blue-500' to={'/'}>Back</Link>
                <div className="flex items-center justify-between mb-4">
                    ...
                    <Link to={`/add-question/${id}`}>
                        <button
                            className="p-2 ml-3 text-sm font-medium text-white bg-blue-700 rounded-lg w-28 focus:ring-4 focus:ring-blue-300 focus:outline-none">
                            Add
                        </button>
                    </Link>
                </div>
                <div>
                    {(showQuestion || []).map((item, index) => {
                        return (
                            <div className="items-center mb-4 md:flex md:justify-between" key={index}>
                               
                                <div className="flex flex-wrap items-center justify-start">
                                <div className="h-20 overflow-hidden md:w-20 md:h-20 w-44 ">
                               {item.image_url &&                       
                                <img className="object-cover w-full h-full rounded" src={process.env.REACT_APP_BASE_API+"/images/"+item.image_url} alt="photoQuestion"/>                        
                                }
